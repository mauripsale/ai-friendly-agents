...
def process_documents(
    ...
    if debug:
        ...
        print(
            f"Ignore images in PDFs (token optimization): {Color.YELLOW}{ignore_images}{Color.END}"
        )
        print(
            f"Output destination: {Color.YELLOW}{output_file if output_file else 'stdout'}{Color.END}"
        )
        Color.list_ten_typical()
        print(
            f"{Color.MAGENTA}Note on google-adk: The google-adk functionality will be added to a future agent.py and not used by this script.{Color.END}"
        )
    ...
    if not files_to_process:
        print(
            f"{Color.YELLOW}🤔 No supported files (PDF, MD, TXT) found in '{input_folder}'. Sad trombone... 🎺{Color.END}"
        )
        raise typer.Exit(code=1)

    if debug:
        print(
            f"{Color.GREEN}Found {len(files_to_process)} files to process:{Color.END}"
        )
    ...
    for i, file_path in enumerate(files_to_process):
        ...
        if debug:
            print(
                f"{Color.BLUE}Processing ({i+1}/{len(files_to_process)}): {file_path.name}...{Color.END}"
            )
        ...
        try:
            ...
            if content is not None:
                ...
                if debug:
                    print(
                        f"{Color.YELLOW}Skipping {file_path.name} due to parsing error or no content.{Color.END}"
                    )
                failed_files += 1

        except Exception as e:
            print(
                f"{Color.RED}😱 Oh no! An unexpected error occurred while processing {file_path.name}: {e}{Color.END}"
            )
            if debug:
                import traceback
    ...
    if final_markdown:
        if output_file:
            try:
                ...
                print(
                    f"{Color.GREEN}🎉 Success! Combined markdown written to: {output_file}{Color.END}"
                )
            except Exception as e:
                print(
                    f"{Color.RED}💥 Error writing to output file {output_file}: {e}{Color.END}"
                )
                if debug:
                    print(
                        f"{Color.YELLOW}Dumping to stdout instead due to file error:{Color.END}\n{final_markdown}"
                    )
                raise typer.Exit(code=1)
        else:
    ...
    if debug:
        ...
        print(
            f"Failed or no content: {Color.RED if failed_files > 0 else Color.GREY}{failed_files}{Color.END}"
        )
